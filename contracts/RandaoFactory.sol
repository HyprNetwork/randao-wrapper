...
contract RandaoWrapperFactory {
    uint256 public randaoWrapperNums = 0;
    mapping(address => address) public randaoWrapperInstancesMap;
    address[] public randaoWrapperInstancesArray;

    event LogContractCreated(address indexed newContract);

    function createContract(address randao, address admin, uint256 _deposit, uint256 _bounty, uint256 _maxTxFee, bool isCheck) public returns (address) {
        RandaoWrapper newContract = new RandaoWrapper(randao, admin, _deposit, _bounty, _maxTxFee, isCheck);
